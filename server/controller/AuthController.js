...
export const registerUser = async (req, res, next) => {
  try {
    ...
    if(!username || !password || !firstName || !lastName )
      return next(new ErrorResponse('Please provide all mandatory fields', 400));
...
export const refreshAccessToken = async (req, res, next) => {
  try {
    ...
    if (!user)
      ...
      user.tokens = user.tokens.concat({ accessToken: newAccessToken, refreshToken: newRefreshToken });
